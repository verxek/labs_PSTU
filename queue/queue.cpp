...
void push(Queue*& queue, int data)   // добавление элемента в очередь
{
	Node* tmp = new Node;
	tmp->data = data;
	tmp->next = nullptr;
...
void pop(Queue*& queue)  // удаление элемента очереди
{
	if (queue->size == 0)
	{
		return;
...
void insert_element(Queue*& queue, int pos, int data) // вставка одного элемента в очередь на указанную позицию
{
	Queue* tmp = new Queue;
	tmp->init();
	for (int i = 1; i < pos; i++)
...
void insert_elements(Queue*& queue, int pos, int k)  // вставка k элементов в очередь на указанную позицию
{ 
	Queue* tmp = new Queue;
	tmp->init();
	int data;
...
void delete_element(Queue*& queue, int pos)  //  удаление одного элемента с заданной позиции
{
	Queue* tmp = new Queue;
	tmp->init();
	for (int i = 1; i < pos; i++)
...
void delete_elements(Queue*& queue, int pos, int k) //  удаление k элементов с заданной позиции
{
	Queue* tmp = new Queue;
	tmp->init();
	for (int i = 1; i < pos; i++)
...
void search_element(Queue*& queue, int element)  // поиск заданного элемента в очереди
{
...
Queue* createQueue(int n)  // создание очереди
{
...
void printQueue(Queue*& queue)  // вывод очереди
{
	Node* curr = new Node;
	curr = queue->front;
