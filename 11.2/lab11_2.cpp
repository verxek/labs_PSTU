...
struct ListElem  // элемент списка
{
    int data;
    ListElem* next;
    ListElem* prev;
...
ListElem* createList(int n)   // создание двунаправленного списка
{
    ListElem* start;
    ListElem* prev, * curr;
    start = new ListElem;
...
void insert_element(ListElem*& start, int pos, int k, int& size) // вставка одного элемента на заданную позицию
{
    size++;
...
void insert_elements(ListElem*& list, int pos, int k, int& size)   // вставка k элементов на заданную позицию
{
    for (int i = 0; i < k; i++)
    {
...
void search_element(ListElem* start, int element)  // поиск элемента в списке
{
    ListElem* curr = start;
    int pos = 0;
